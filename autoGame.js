...
const autoGame = async () => {
    ...
    exp.move(track).then(() => {
        exp.outGame().then(async (res) => {
            ...
            response += `<p>Game over, Reward: ${res.body.data.realDiamond}, Today reward: ${res.body.data.todayDiamond}, Today limit reward: ${res.body.data.todayLimitDiamond}</p>`
            ...
            if (res.body.data.todayDiamond < res.body.data.todayLimitDiamond) {
                ...
                response += "<p>今日奖励已达上限！</p>"
                sendEmail("掘金自动化-海底挖矿" , response)
